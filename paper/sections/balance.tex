...
catalogued non-commutative rank over \(\Reals\) (catalog snapshot 2026-06).
...
\begin{axis}[
  ...
  xlabel={imbalance \(p-n\)}, ylabel={median \(\omega\)},
  xmin=-0.5, xmax=14.5, ymin=2.810, ymax=2.872,
  xtick={0,2,4,6,8,10,12,14},
  ytick={2.81,2.82,2.83,2.84,2.85,2.86,2.87},
  grid=both, grid style={gray!20}, tick label style={font=\small},
  label style={font=\small}, mark size=1.6pt]
\addplot[blue, thick, mark=*] coordinates {
  (0,2.8155) (1,2.8148) (2,2.8177) (3,2.8180) (4,2.8169) (5,2.8193)
  (6,2.8214) (7,2.8229) (8,2.8274) (9,2.8299) (10,2.8332) (11,2.8402)
  (12,2.8536) (13,2.8616) (14,2.8695)};
\end{axis}
\end{tikzpicture}
\caption{The data of Table~\ref{tab:omega-imbalance} as a curve: median
\(\omega\) is flat (\(\approx 2.817\)) out to imbalance \(\sim 7\), then
climbs steeply --- the penalty for imbalance is convex, not linear.}
\label{fig:omega-imbalance}
\end{figure}

The median rises from \(2.8155\) (balanced) to \(2.8695\) at imbalance \(14\).
This is the average-case fact the avoid-unbalanced heuristic rests on.
...
\(2{\times}2{\times}2\) split realises one all-max corner, three two-max
corners, and three one-max corners). Reading the \(\Delta\) column top to
bottom is the whole story: the unbalanced \((6,3)^3\) split \emph{saves \(63\)}
on the three one-max corners (the \(\nmpshape{3}{3}{6}\) gems, \(40\) vs \(61\))
but is mechanically forced to \emph{pay \(+60\)} on the single all-max corner
(\(\nmpshape{6}{6}{6}{=}153\) instead of \(\nmpshape{5}{5}{5}{=}93\)) plus
\(+12\) on the middle tier --- net \(+9\), so balance wins here.}
\label{tab:9-faceoff}
...
all-max corner} \((\max,\max,\max)\). The unbalanced split buys three cheap
\(\nmpshape{3}{3}{6}\)-type corners (saving \(3\times(61-40)=63\)) but
mechanically forces the all-max corner from \(\nmpshape{5}{5}{5}{=}93\) up to
\(\nmpshape{6}{6}{6}{=}153\) (costing \(+60\)), plus \(+12\) on the middle
corners --- net \(+9\). Because \(R\) grows like \(\text{size}^{\,\omega}\) with
\(\omega \approx 2.8\), pushing the max block from \(5\) to \(6\) costs more
than the cheap corners recover. The efficiency of \(\nmpshape{3}{3}{6}\) is
real, but a single block split cannot isolate it from its complementary
all-max corner.
...
a complementary max-corner along. (A Sch\"onhage \(\tau\)-disjoint sum would
compound the ratio in the same way, but we do not use that route --- see
Section~\ref{ssec:disjoint-sum}.)
...
base the \emph{exact} rank-minimising allocation by branch-and-bound
(\code{AllocationOptimizer}: balanced incumbent + the admissible lower bound of
Section~\ref{ssec:omega-not-extractable}, verified against exhaustive search),
...
touch that unit block compute \(\nmpshape{\cdot}{\cdot}{1}\) sub-shapes, whose
rank is the naive product (here \(R = 4, 6, 6, 9\)), while the remaining
products route through the cheap \(\nmpshape{3}{3}{6}{=}40\) corner. The thin
strip costs almost nothing yet lets the rest land on the efficient
\(\nmpshape{3}{3}{6}\) family --- the unbalanced-extraction mechanism in its
purest form, and precisely the allocation a balanced-only rule forbids.
